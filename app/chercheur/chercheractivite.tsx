...
const ChercherActivite = () => {
    ...
    const fetchActivitiesByCode = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.EXPO_PUBLIC_API_URL}/projets/activite/${activityCode}`, {
                    ...
                    <Div flex={1} justifyContent="center" alignItems="center">
                        {activities.map((activity) => (
                            <Button
                                ...
                                borderWidth={1}
                                borderColor="gray300"
                                opacity={activity.status === "disabled" ? 0.7 : 1}
                                disabled={activity.status === "disabled"}
                                onPress={() =>
                                    router.push(
                                        `/chercheur/insertiondedonnee?megaprojet_id=${activity.megaprojet_id}&megaprojet_name=${activity.megaprojetName}&axe_id=${activity.axe_id}&axe_name=${activity.axeName}&activite_id=${activity._id}&activite_name=${activity.ACTIVITE}`
                                    )
                                }
                            >
